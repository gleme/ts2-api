...
const {
    MedicalDiagnosis,
    Category
} = require('../../src/domain/model/medical-diagnosis');
...
(async () => {
    try {
        ...
        await destDb.collection('cid10').deleteMany({});
        console.log('destination database empty');

        /**
         * Transformation script
         */
        sourceDb.collection('SubCategory')
            .find({})
            .forEach(
                async subCat => {
                        try {
                            const {
                                ...
                                // find category
                                category: callback => {
                                    sourceDb.collection('Category').findOne({
                                            code: subCat.group
                                        })
                                ...
                                details: async callback => {
                                    try {
                                        ...
                                        if (!details) {
                                            details = await sourceDb.collection('DETAIL')
                                                .findOne({
                                                    CID_STAMPS: {
                                                        $elemMatch: {
                                                            $eq: subCat.group
                            ...
                            // assembly diagnosis
                            const diagnosis = new MedicalDiagnosis(subCat.code, subCat.description, category, symptoms);
                            diagnosis.info = get(details, 'info', '');


                            // input in destination database
                            destDb.collection('cid10').insertOne(diagnosis, () => {
